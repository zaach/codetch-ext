...
const LINE_COMMENT= /\/\/[^\n]*\n/;
const BLOCK_COMMENT= /\/\*[^*]*\*+([^\/][^*]*\*+)*\//;
const SGML_COMMENT= /<!\s*(--([^-]|[\r\n]|-[^-])*--\s*)>/;
const CDATA= /<\!\[[\w\s]*?\[(.|\s)*?\]\]>/;
const STRING1= /'[^']*'/;
const STRING2= /"[^"]*"/;
const NUMBER= /\b[+-]?(\d*\.?\d+|\d+\.?\d*)([eE][+-]?\d+)?\b/;
...
function _stylise($replacement, $style) {
	return $replacement.replace(/span>/g, "span style='" + $style + "'>");
};
